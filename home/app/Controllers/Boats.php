...
public function openTripRequest()
{
    ...
    // Dapatkan kapasitas kapal
    $boatId = $this->request->getPost('boat_id');
    $boat = $boatModel->find($boatId);
    
    if (!$boat) {
        return $this->response->setStatusCode(400)->setJSON(['error' => 'Kapal tidak ditemukan']);
    }
...
public function cancelRequest()
{
    ...
    if ($request['user_id'] != $_SESSION['userData']['user_id'] && $_SESSION['userData']['role'] != 'admin') {
        return $this->response->setJSON(['success' => false, 'error' => 'Unauthorized']);
    }
    
    // Check if request is already approved
    if ($request['status'] == 'approved') {
        return $this->response->setJSON(['success' => false, 'error' => 'Cannot cancel approved request']);
    }
